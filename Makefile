...
cs-fix:
	docker exec bank-app ./vendor/bin/pint

composer-install:
	docker exec bank-app composer install

cp-env:
	docker exec bank-app cp .env.example .env

artisan-key-generate:
	docker exec bank-app cp php artisan key:generate

artisan-migrate:
	docker exec bank-app cp php artisan migrate
art:
	docker exec bank-app php artisan

phpstan:
	docker exec bank-app ./vendor/bin/phpstan analyse --memory-limit=2G
##
# up
#
